...
标注和品种库是两个独立的业务模块，共享 `stocks` 表但语义完全不同：

| 维度 | 品种库模块 | 标注模块 |
|------|-----------|---------|
| **截止日期 (end_date)** | 无（NULL），代表实时最新数据 | 必填，代表标注时刻的历史快照 |
| **评级来源** | 纯算法评级，不受人工标注影响 | COALESCE(人工标注, 算法评级)，人工优先 |
| **状态机** | 参与状态流转（pending → idle → watching → focused → holding / removed） | 不参与，watch_status 固定为 none |
| **用途** | 驱动自动化监控、升降级、模拟下单 | 历史回溯、标注记录归档 |

**核心设计**：
- 品种库中的品种始终反映最新算法分析结果，状态机升降级判断完全基于算法评级
- 当用户在品种库详情页进行人工标注时，系统自动创建一条独立的**快照记录**存入标注模块，end_date 自动设为当天日期
- 同一股票在标注模块中允许有多条不同截止日期的记录，形成标注历史轨迹
- 品种库原始记录不受人工标注操作影响，保持纯算法数据
