...
基于小时K线的六维量化开仓条件分析引擎。通过 DL/PT/LK/SF/TY/DN 六个维度对股票进行系统化评估，判断是否满足开仓条件并给出仓位建议。支持 A股/港股/美股。
...
├── docs/                       # 设计文档
│   ├── TRADING_RULES.md        # 交易规则说明（代码逻辑的唯一来源）
│   └── architecture.md         # 项目架构设计
├── tests/                      # 测试
│   ├── regression_test.py      # 回归测试（算法 vs 人工标注）
│   ├── test_api.py             # API 测试
│   └── ...
├── data/                       # 数据目录
...
| 维度 | 名称 | 说明 | 评分 |
|------|------|------|------|
| **DL** | 独立结构 | 识别盘整区间，检测价格是否形成有效的横盘调整结构 | S/C |
| **PT** | 平台位 | 检测阻力位和支撑位区间，评估触碰次数、间隔分布和穿越情况 | S/A/B/C |
| **LK** | 轮廓质量 | 评估调整形态的视觉质量：对称性、密集度、尾部破位、波峰均匀性、单边趋势检测 | S/A/B/C |
| **SF** | 释放级别 | 评估尾部是否向突破方向蹭动消耗动能，含退化规则和PT距离检测 | 1st/2nd/3rd |
| **TY** | 挤压收敛 | 检测调整末尾K线实体是否收敛挤压（基于ATR，严格模式用body度量） | S/A/B/C/待定 |
| **DN** | 动能突破 | 检测是否出现突破触发信号，评估力度比和放量比 | S/A/B/C/待定 |
...
- **数据导出**: 导出全部或按标签导出 CSV，标签修改实时同步到 `labeled_cases.csv`
- **单只重分析**: 修改截止日期后可对单只股票触发重新分析
- **回归测试**: 72个标注案例，算法准确率 100%
